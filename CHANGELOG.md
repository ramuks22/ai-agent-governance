...
- CLI command `upgrade` with conflict-aware planning, optional deterministic patch output, and manifest migration to v2.
- CLI command `rollback` with backup index targeting (`--to <backup-id>`), dry-run support, and clean-tree safety checks.
- Managed-block file strategy for markdown/yaml/shell/hook artifacts with deterministic doctor validation.
- Backup snapshot index at `.governance/backups/index.json` for restore auditability.
