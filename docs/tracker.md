...
| AG-GOV-006 | High     | Governance | **Workshop applicability friction risk**: overhead can rise if teams cannot quickly decide when workshop is required.    | In active implementation: docs-only governance enhancement (workshop exempt by applicability rules), required applicability evidence line in PR body, and tracker-to-PR audit linkage. | Add a concise applicability decision checklist and examples to keep feature triage under one minute.                        | Implementation | In Progress |
| AG-GOV-007 | High     | Governance | **Governance drift risk**: cross-doc workshop rules can diverge over time.                                               | Requirement list `R-002` + `DEP-002` (cross-doc consistency requirement).                                                                                                              | Define a periodic doc-consistency review cadence and evidence expectations in tracker notes.                                | Idea           | Not Started |
| AG-GOV-008 | High     | Governance | **Exception ambiguity risk**: hotfix exceptions can become non-auditable if approval fields are inconsistently captured. | Requirement list `R-003` + `FR-007` + `AC-006` (approval + due date + retroactive SLA).                                                                                                | Standardize exception evidence format (reason, approvers, due date, closure evidence) across tracker/PR usage.              | Idea           | Not Started |
