...
| AG-GOV-007 | High     | Governance | **Governance drift risk**: cross-doc workshop rules can diverge over time.                                               | Applicability: Not Required — Reason: documentation-only governance process definition (all four applicability checks are No; exempt as documentation-only changes). Phase path in progress: Idea -> Discovery -> Implementation -> Validation. | Define a periodic doc-consistency review cadence and evidence expectations in tracker notes.                                | Validation | In Progress |
