...
| AG-GOV-008 | High     | Governance | **Exception ambiguity risk**: hotfix exceptions can become non-auditable if approval fields are inconsistently captured. | Docs-only governance change (workshop exempt; all four applicability checks = No; exempt as documentation-only changes). Implementation in progress: canonical 4-field contract and cross-doc alignment updates in governance workflow, workshop workflow, delivery governance, and PR template. | Standardize exception evidence format (reason, approvers, due date, closure evidence) across tracker/PR usage.              | Validation | In Progress |
